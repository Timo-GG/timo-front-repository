    ...
    <>
      ...
      <Section label="듀오 찾기">
        ...
        <ButtonBox
          label="듀오 스타일"
          items={[
            { label: '친목/잡담', value: 'FRIEND' },
            { label: '오더/보이스', value: 'ORDER' },
            { label: '즐겜/빡겜', value: 'FUN' },
          ]}
          type="duoStyle"
          onClick={handleClick}
          state={state}
          ...
          {positionIcon.map((icon, index) => (
            <div
              ...
              className={`px-2 py-2 flex justify-center items-center ${onHover} ${
                state[type] === positionValue[index] ? 'bg-secondary-green' : ''
              }`}
              onClick={() => onClick(type, positionValue[index])}
            >
              {icon}
            </div>
          ))}
